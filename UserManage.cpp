...
//创建一个新的对象
bool UserManage::createUser(string name, string password, string email,
                            string phone) {
  //如果名字存在，则不再创建
  if (findUserByName(name) != NULL) return false;
...
//返回所有用户信息
list<User> UserManage::listAllUsers() { return users; }

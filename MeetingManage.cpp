...
//新加一个会议
bool MeetingManage::addMeeting(string sponsor, string participator, Date stime,
                               Date etime, string title) {
  ...
  for (it = meetings.begin(); it != meetings.end(); it++) {
    ...
    if (((*it).getParticipator() == participator) &&
        (!((*it).getStartDate() > etime || (*it).getEndDate() < stime)))
      return false;
    //与受邀者时间冲突，加入无效
...
//更新信息
bool MeetingManage::updateMeetingParticipator(string title,
                                              string newParticipator) {
  list<Meeting>::iterator it;
  //根据指定主题更新会议的受邀者信息
  for (it = meetings.begin(); it != meetings.end(); it++) {
    //发起者时间冲突，加入无效
    if (((*it).getParticipator() == newParticipator) &&
        (!((*it).getStartDate() > queryMeetingByTitle(title)->getEndDate() ||
           (*it).getEndDate() < queryMeetingByTitle(title)->getStartDate())))
      return false;
    //与受邀者时间冲突，加入无效
    if (((*it).getSponsor() == newParticipator) &&
        (!((*it).getStartDate() > queryMeetingByTitle(title)->getEndDate() ||
           (*it).getEndDate() < queryMeetingByTitle(title)->getStartDate())))
      return false;
  }
...
list<Meeting> MeetingManage::queryMeetingsByTime(string userName, Date stime,
                                                 Date etime) {
  ...
  for (it = meetings.begin(); it != meetings.end(); it++) {
    if ((((*it).getSponsor() == userName) ||
         ((*it).getParticipator() == userName)) &&
        !((*it).getStartDate() >= etime || (*it).getEndDate() <= stime))
      MeetingBytime.push_back(*it);
  }
  if (stime >= etime) MeetingBytime.clear();
...
list<Meeting> MeetingManage::listAllSponsorMeetings(string userName) {
  ...
  //根据名字列出他所有发起的会议
  for (it = meetings.begin(); it != meetings.end(); it++) {
    if ((*it).getSponsor() == userName) AllSponsorMeetings.push_back(*it);
  }
...
bool MeetingManage::load() {
  ...
  while (infile >> sponsor >> participator >> stime >> etime >> title) {
    addMeeting(sponsor, participator, Date::convertStringToDate(stime),
               Date::convertStringToDate(etime), title);
  }
  infile.close();
  return true;
}
...
bool MeetingManage::save() {
  ...
  if (outfile) {
    for (list<Meeting>::iterator it = meetings.begin(); it != meetings.end();
         it++) {
      outfile << it->getSponsor() << ' ' << it->getParticipator() << ' '
              << Date::convertDateToString(it->getStartDate()) << ' '
              << Date::convertDateToString(it->getEndDate()) << ' '
              << it->getTitle() << endl;
    }

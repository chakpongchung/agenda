...
void AgendaController::listAllUsers() {
  if (currentUser != NULL) {
    ...
    userslist = agendaService.listAllUsers();  //ͨ��Service�㸳ֵUser�б�
                                               //��ӡ��ͷ
    printf("\n");
    printf("[list all users]\n");
    cout << left << setw(8) << "name" << setw(17) << "email" << setw(11)
         << "phone" << endl;
    //��ӡ�б�
    for (it = userslist.begin(); it != userslist.end(); it++)
      cout << left << setw(8) << it->getName() << setw(17) << it->getEmail()
           << setw(11) << it->getPhone() << endl;
...
void AgendaController::createMeeting() {
  if (currentUser != NULL) {
    cout << endl;
    cout << "[create meeting] [title] [participator] [start "
            "time(yyyy-mm-dd/hh:mm)] [end time(yyyy-mm-dd/hh:mm)]"
         << '\n'
         << "[create meeting] ";
    ...
    if ((userManage->findUserByName(participator)) != NULL) {
      //�ɹ�create
      if (agendaService.createMeeting(currentUser->getName(), title,
                                      participator,
                                      Date::convertStringToDate(stime),
                                      Date::convertStringToDate(etime))) {
...
void AgendaController::listAllMeetings() {
  if (currentUser != NULL) {
    ...
    cout << left << setw(13) << "title" << setw(13) << "sponsor" << setw(17)
         << "participator" << setw(17) << "start time" << setw(17) << "end time"
         << endl;
...
void AgendaController::queryMeetingByTitle() {
  if (currentUser != NULL) {
    ...
    if ((meeting = agendaService.meetingQuery(title)) != NULL) {
      cout << left << setw(15) << meeting->getSponsor() << setw(15)
           << meeting->getParticipator() << setw(25)
           << Date::convertDateToString(meeting->getStartDate()) << setw(25)
...
void AgendaController::queryMeetingByTimeInterval() {
  if (currentUser != NULL) {
    ...
    if (stime > etime || stime == etime) {
      ...
      cout << left << setw(13) << "title" << setw(13) << "sponsor" << setw(17)
           << "participator" << setw(17) << "start time" << setw(17)
           << "end time" << endl;
      //��ӡ�����б�
      printMeetings(agendaService.meetingQuery(
          currentUser->getName(), Date::convertStringToDate(stime),
          Date::convertStringToDate(etime)));
      getOperation();
    }
  }
...
void AgendaController::printMeetings(list<Meeting> meetings) {
  ...
  //�б���Ϊ��
    ...
    for (it = meetings.begin(); it != meetings.end(); it++) {
      // ��ӡ
      cout << left << setw(13) << it->getTitle() << setw(13) << it->getSponsor()
           << setw(17) << it->getParticipator() << setw(17)
           << Date::convertDateToString(it->getStartDate()) << setw(17)
           << Date::convertDateToString(it->getEndDate()) << endl;
    }
  }
}

AgendaController::AgendaController() { currentUser = NULL; }
...
void AgendaController::updateMeetingStartDate() {
  if (currentUser != NULL) {
    ...
    if (agendaService.meetingQuery(title) != NULL) {
      if (Date::isValid(Date::convertStringToDate(startDate)) &&
          Date::convertStringToDate(startDate) <
              (agendaService.meetingQuery(title))->getEndDate()) {
        list<Meeting> smeeting;
        list<Meeting> pmeeting;
        MeetingManage *meetingManage = MeetingManage::getInstance();

        pmeeting = agendaService.meetingQuery(
            (meetingManage->queryMeetingByTitle(title))->getParticipator(),
            Date::convertStringToDate(startDate),
            (agendaService.meetingQuery(title))->getStartDate());

        smeeting = agendaService.meetingQuery(
            currentUser->getName(), Date::convertStringToDate(startDate),
            (agendaService.meetingQuery(title))->getStartDate());
...
void AgendaController::updateMeetingEndDate() {
  if (currentUser != NULL) {
    ...
    if (agendaService.meetingQuery(title) != NULL) {
      if (Date::isValid(Date::convertStringToDate(endDate)) &&

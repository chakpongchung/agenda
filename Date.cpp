...
bool Date::isValid(Date date) {
  ...
  if (date.month > 12 || date.month < 1)
    ...
    if (date.minute < 0 || date.minute > 60)
      ...
      if (date.day > leap[date.month - 1]) return false;
    } else if (!isLeap) {
      if (date.day > nleap[date.month - 1]) return false;
    } else
      return true;
    return true;
  }
...
Date Date::convertStringToDate(string ds) {
  ...
  */
  Date d;
  d.year = (ds[0] - 48) * 1000 + (ds[1] - 48) * 100 + (ds[2] - 48) * 10 +
           (ds[3] - 48);
...
string Date::convertDateToString(Date date) {
  ...
  tmp = ToString(y) + "-" + ToString(mo) + "-" + ToString(d) + "/" +
        ToString(h) + ":" + ToString(mi);
  return tmp;
}
...
bool Date::operator==(const Date &t) const {
  if (year == t.getYear() && month == t.getMonth() && day == t.getDay() &&

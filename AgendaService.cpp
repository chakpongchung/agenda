...
//析构函数
AgendaService::~AgendaService() { Save(); }
...
User *AgendaService::userLogIn(string userName, string password) {
  User *u;
  //账号密码相对  返回用户指针
  if ((u = userManage->findUserByName(userName)) != NULL &&
...
// 用户注册
bool AgendaService::userRegister(string userName, string password, string email,
                                 string phone) {
...
// listAllUsers
list<User> AgendaService::listAllUsers() { return userManage->listAllUsers(); }
// createMeeting
bool AgendaService::createMeeting(string userName, string title,
                                  string participator, Date stime, Date etime) {
...
// meetingQuery
list<Meeting> AgendaService::meetingQuery(string userName, Date stime,
                                          Date etime) {
...
bool AgendaService::updateMeetingParticipator(string title,
...
bool AgendaService::updateMeetingStartDate(string title, string newStartDate) {
  return meetingManage->updateMeetingStartDate(
      title, Date::convertStringToDate(newStartDate));
}

bool AgendaService::updateMeetingEndDate(string title, string newEndDate) {
  return meetingManage->updateMeetingEndDate(
      title, Date::convertStringToDate(newEndDate));
}
